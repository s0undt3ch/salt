...
TestsuiteResult = namedtuple(
    "TestsuiteResult", ["header", "errors", "skipped", "failures", "passed"]
)
TestResult = namedtuple("TestResult", ["id", "reason"])


def print_header(
    header, sep="~", top=True, bottom=True, inline=False, centered=False, width=PNUM
):
    """
...
class SaltTestingParser(optparse.OptionParser):
    ...
    def __init__(self, testsuite_directory, *args, **kwargs):
        ...
        xml_output_dir_env_var = kwargs.pop(
            "xml_output_from_env", "XML_TESTS_OUTPUT_DIR"
        )
        ...
        self.test_selection_group = optparse.OptionGroup(
            self, "Tests Selection Options", "Select which tests are to be executed"
        )
        ...
        self.test_selection_group.add_option(
            ...
            default=[],
            help=(
                "Specific test name to run. A named test is the module path "
        ...
        self.test_selection_group.add_option(
            ...
            dest="names_file",
            default=None,
            help=("The location of a newline delimited file of test names to " "run"),
        )
        self.test_selection_group.add_option(
            "--from-filenames",
            dest="from_filenames",
            ...
            default=None,
            help=(
                "Pass a comma-separated list of file paths, and any "
