    ...
    **PLATFORM-SPECIFIC ARGUMENTS**
    ...
    if sources:
        ...
        if kwargs.get('resolve_capabilities', False) and 'pkg.list_provides' in __salt__:
            ...
            new_caps = {}
        good, failed = _verify_install(desired, new_pkgs,
                                       ignore_epoch=ignore_epoch,
                                       new_caps=new_caps)
        modified = [x for x in good if x in targets]
        not_modified = [x for x in good
                        if x not in targets
                        and x not in to_reinstall]
...
def downloaded(name,
    ...
    good, failed = _verify_install(targets, new_pkgs, ignore_epoch=ignore_epoch)
    ...
    **kwargs):
    ...
    if action == 'purge':
        new_removed = __salt__['pkg.list_pkgs'](versions_as_list=True,
                                                removed=True,
                                                **kwargs)
        failed.extend([param for param in pkg_params if param in new_removed])
    failed.sort()
